  ...
  (product: any, index = 0) => {
    try {
      ...
      return {
        ...
        image: product.featuredImage?.url?.replace('https:', '') || '',
        position: index + 1,
      };
    } catch (error: any) {
  ...
  (variant: any, index = 0) => {
    try {
      ...
      return {
        ...
        image: variant.image?.url?.replace('https:', '') || '',
  ...
  (line: {quantity?: number; merchandise?: any; index?: number}, index = 0) => {
    try {
      ...
      return {
        ...
        image: merchandise.image?.url?.replace('https:', '') || '',
...
export const fetchCustomerOrders = () => null;
